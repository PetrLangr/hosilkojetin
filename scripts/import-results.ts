...
// Parsed data from Kola.html - matches with results from rounds 1-8
const matchData = [
  // Kolo 1 - 20.9.2025
  ...
  // Kolo 8 - 22.11.2025 (only one match has result)
...
// Calculate points based on HŠL rules
function calculatePoints(homeGames: number, awayGames: number): { homePoints: number, awayPoints: number } {
  if (homeGames > awayGames) {
    // Home wins
    if (awayGames <= 4) {
      return { homePoints: 3, awayPoints: 0 } // V (victory)
    } else {
      return { homePoints: 2, awayPoints: 1 } // VP/PP (close win/loss)
    }
  } else if (awayGames > homeGames) {
    // Away wins
    if (homeGames <= 4) {
      return { homePoints: 0, awayPoints: 3 } // P (loss)
    } else {
      return { homePoints: 1, awayPoints: 2 } // PP/VP
    }
  } else {
    // Draw (shouldn't happen in HŠL but just in case)
    return { homePoints: 1, awayPoints: 1 }
  }
}

async function main() {
  console.log('Starting import of match results...\n')
  ...
  console.log(`Found active season: ${season.name} (${season.id})\n`)
  ...
  // Create team name mapping (handle variations)
  const teamMap = new Map<string, string>()
  teams.forEach(team => {
    teamMap.set(team.name, team.id)
    // Also add without diacritics or variations
  ...
  for (const match of matchData) {
    ...
    if (!homeTeamId) {
      console.error(`❌ Home team not found: "${match.home}"`)
      errors++
      continue
    }

    if (!awayTeamId) {
      console.error(`❌ Away team not found: "${match.away}"`)
      errors++
      continue
    }
    ...
    if (existing) {
      console.log(`⏭️  Match already exists: ${match.home} vs ${match.away} (round ${match.round})`)
      skipped++
      continue
    }
    ...
    await prisma.match.create({
      data: {
        ...
        endTime: new Date(`${match.date}T18:00:00`), // Required for standings calculation
        status: 'completed',
        isQuickResult: true,
        result: {
          homeLegs: match.homeLegs,
          awayLegs: match.awayLegs,
          homeWins: match.homeGames,  // standings.ts expects homeWins
          awayWins: match.awayGames,  // standings.ts expects awayWins
          homePoints,
          awayPoints
        }
      }
    ...
    console.log(`✅ Created: ${match.home} ${match.homeGames}:${match.awayGames} ${match.away} (round ${match.round})`)
    created++
  }
